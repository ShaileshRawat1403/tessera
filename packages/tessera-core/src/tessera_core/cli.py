...
from tessera_core import __version__
from tessera_core.plugins import load_cli_plugins, load_jobpacks
from tessera_core.workspace import DEFAULT_HOME, ensure_workspace

console = Console()
app = typer.Typer(help="Tessera: reusable workflow tools for AI and engineering jobs.")
...
def init(home: Path = typer.Option(DEFAULT_HOME, help="Workspace directory.")) -> None:
    """Create the local Tessera workspace."""
...
def plugins_cmd() -> None:
    ...
    commands_table = Table(title="CLI Command Plugins (tessera.commands)")
    ...
    packs_table = Table(title="Job Packs (tessera.jobpacks)")
